...
FB::BrowserStreamPtr NpapiBrowserHost::_createPostStreamWithHeader(const std::string& url, const FB::PluginEventSinkPtr& callback,
                                    const std::string& header, const std::string& postdata, bool cache, bool seekable, size_t internalBufferSize ) const
{
    NpapiStreamPtr stream( boost::make_shared<NpapiStream>( url, cache, seekable, internalBufferSize, FB::ptr_cast<const NpapiBrowserHost>(shared_from_this()) ) );
    stream->AttachObserver( callback );
    ...
    // always use target = 0 for now
    if ( PostURLNotify( url.c_str(), 0, request.str().length(), request.str().c_str(), false, stream.get() ) == NPERR_NO_ERROR )
    {
        stream->setCreated();
        StreamCreatedEvent ev(stream.get());
        stream->SendEvent( &ev );
